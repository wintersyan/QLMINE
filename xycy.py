...
#获取用户信息
def get_user(token):
    url = "https://s76.yyyyy.run/api/user"
    headers = {
...
def xycy_token():  # 方法 获取 xycy_tk [系统变量传递]
    if "xycy_tk" in os.environ:  # 判断 xycy_tk
        token_list = os.environ['xycy_tk'].split('&')  # 读取系统变量 以 & 分割变量
        if len(token_list) > 0:  # 判断 xycy_tk 数量 大于 0 个
            return token_list  # 返回 xycy_tk [LIST]
        else:  # 判断分支
            logger.info("xycy_tk变量未启用")  # 标准日志输出
            sys.exit(1)  # 脚本退出
    else:  # 判断分支
        logger.info("未添加xycy_tk变量")  # 标准日志输出
        sys.exit(0)  # 脚本退出        
